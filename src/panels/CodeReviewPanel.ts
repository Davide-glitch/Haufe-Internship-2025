...
import { ReviewResult } from '../services/ReviewManager';
...
export class CodeReviewPanel {
  ...
  static render(extensionUri: vscode.Uri, review: ReviewResult, context: vscode.ExtensionContext, llmRouter: LLMRouter) {
  ...
  private update(review: ReviewResult) {
    this.panel.title = 'AI Code Review';
    this.panel.webview.html = this.getHtml(review);
  }

  private getHtml(review: ReviewResult): string {
