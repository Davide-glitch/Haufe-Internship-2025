...
export function activate(context: vscode.ExtensionContext) {
  ...
  let promptPanel: vscode.WebviewPanel | undefined;

  const API_BASE = process.env.AI_REVIEW_API || 'http://localhost:7070';

  const openQuickPrompt = (preserveFocus = true) => {
    if (promptPanel) {
      try { promptPanel.reveal(vscode.ViewColumn.Two, preserveFocus); } catch {}
      return;
    }
    ...
    promptPanel.webview.onDidReceiveMessage(async (message) => {
      ...
      if (message.type === 'review') {
        try {
          ...
          const r = await axios.post(`${API_BASE}/api/review`, { code, language, model }, { timeout: 60_000 });
    ...
    await vscode.window.withProgress(
      ...
      async (progress, token) => {
        try {
          ...
          CodeReviewPanel.render(context.extensionUri, review, context, llmRouter);
        } catch (error: any) {
  ...
  const applyFixCommand = vscode.commands.registerCommand('aiCodeReview.applyFix', async (args?: any) => {
    try {
      if (!args) return;
  ...
  try {
    ...
    const reviewed = new WeakSet<vscode.TextDocument>();

    const isSupported = (doc?: vscode.TextDocument | null) => {
      if (!doc) return false;
      const lang = doc.languageId;
      return lang === 'python' || lang === 'javascript' || lang === 'typescript';
    };

    const runAuto = async (doc?: vscode.TextDocument | null) => {
      if (!autoRun) return;
      const target = doc ?? vscode.window.activeTextEditor?.document;
      if (!target || !isSupported(target)) return;
      if (reviewed.has(target)) return;
      reviewed.add(target);
      try {
        await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'AI Code Review (auto)…', cancellable: false },
          async () => {
    ...
    context.subscriptions.push(
      vscode.workspace.onDidOpenTextDocument((d) => {
        if (isSupported(d)) setTimeout(() => runAuto(d), 200);
      })
    );
    ...
    context.subscriptions.push(
      vscode.window.onDidChangeActiveTextEditor((ed) => {
        if (ed && isSupported(ed.document)) setTimeout(() => runAuto(ed.document), 200);
      })
    );
  } catch (e) {
